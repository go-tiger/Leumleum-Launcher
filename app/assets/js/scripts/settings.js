...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        titleElement.innerHTML = '프리-릴리즈'
        titleElement.style.color = '#ff886d'
        checkElement.style.background = '#ff886d'
    } else {
        titleElement.innerHTML = '안정된 버전'
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/go-tiger/Leumleum-Launcher/releases.atom',
        success: (data) => {
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        if(process.platform === 'darwin'){
            ...
            settingsUpdateButtonStatus('설치 중..', true)
        }
    } else {
        settingsUpdateTitle.innerHTML = '현재 최신 버전을 실행 중입니다.'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('업데이트 확인하기', false, () => {
            if(!isDev){
                ipcRenderer.send('autoUpdateAction', 'checkForUpdate')
                settingsUpdateButtonStatus('업데이트를 확인 중입니다...', true)
            }
        })
    }
}
